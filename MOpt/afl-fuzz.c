...
static u64 stage_finds_puppet[swarm_num][operator_num], /* Patterns found per
                                                           fuzz stage    */
...
static void destroy_extras(void) {
  ...
  for (i = 0; i < extras_cnt; i++) ck_free(extras[i].data);

  ck_free(extras);

  for (i = 0; i < a_extras_cnt; i++) ck_free(a_extras[i].data);
    ...
    if (key_puppet == 0) {
      if (key_module == 0) {
        sprintf(tmp + banner_pad, "%s " cLCY VERSION cLGN " (%s)",
                crash_mode ? cPIN "peruvian were-rabbit"
                           : cYEL "MOpt-AFL (pilot_fuzzing)",
                use_banner);
      } else if (key_module == 1) {
        sprintf(tmp + banner_pad, "%s " cLCY VERSION cLGN " (%s)",
                crash_mode ? cPIN "peruvian were-rabbit"
                           : cYEL "MOpt-AFL (core_fuzzing)",
                use_banner);
      } else if (key_module == 2) {
        sprintf(tmp + banner_pad, "%s " cLCY VERSION cLGN " (%s)",
                crash_mode ? cPIN "peruvian were-rabbit"
                           : cYEL "MOpt-AFL (pso_updating)",
                use_banner);
      }
    } else {
      if (key_module == 0) {
        sprintf(tmp + banner_pad, "%s " cLCY VERSION cLGN " (%s)",
                crash_mode ? cPIN "peruvian were-rabbit"
                           : cYEL "MOpt-AFL + pacemaker (pilot_fuzzing)",
                use_banner);
      } else if (key_module == 1) {
        sprintf(tmp + banner_pad, "%s " cLCY VERSION cLGN " (%s)",
                crash_mode ? cPIN "peruvian were-rabbit"
                           : cYEL "MOpt-AFL + pacemaker (core_fuzzing)",
                use_banner);
      } else if (key_module == 2) {
        sprintf(tmp + banner_pad, "%s " cLCY VERSION cLGN " (%s)",
                crash_mode ? cPIN "peruvian were-rabbit"
                           : cYEL "MOpt-AFL + pacemaker (pso_updating)",
                use_banner);
      }
    }
  } else {
    sprintf(tmp + banner_pad, "%s " cLCY VERSION cLGN " (%s)",
            crash_mode ? cPIN "peruvian were-rabbit" : cYEL "AFL", use_banner);
  }
...
static void usage(u8* argv0) {
  SAYF(
      ...
      "  -L            - use MOpt-AFL and set the limit time for entering the "
      "pacemaker fuzzing mode (set 0 will enter pacemaker mode at first). For "
      "instance, (-L 30): if MOpt-AFL finishes the mutation of one input while "
      "does not find any interesting test case for more than 30 min, MOpt-AFL "
      "will enter the pacemaker fuzzing mode (it may take three or four days "
      "for MOpt-AFL to enter the pacemaker fuzzing mode when '-L 30').\n\n"
...
int main(int argc, char** argv) {
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:b:t:V:T:L:dnCB:S:M:x:Q")) > 0)
  ...
  while (1) {
    ...
    if (most_time_key == 1) {
      if (most_time_puppet * 1000 < cur_ms_lv - start_time) break;
    }
  }
